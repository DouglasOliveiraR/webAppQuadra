...
function PremioCard({ titulo, icone, imgSrc, subtitulo, pontos, estilo, vencedores }) {
  return (
    <div className={`bento-card bg-gradient-to-br ${estilo} border border-outline/10 p-6 flex flex-col justify-between min-h-[180px] transition-all duration-300 hover:-translate-y-1 hover:shadow-md cursor-default group`}>
      <div>
        <div className="flex justify-between items-start mb-3">
          <div className="flex items-center gap-3">
            {imgSrc ? (
              <img src={imgSrc} alt={titulo} className="w-12 h-12 object-contain drop-shadow-md group-hover:scale-110 transition-transform duration-300" />
            ) : (
              <span className="material-symbols-outlined text-[32px] icon-fill text-surface group-hover:scale-110 transition-transform duration-300" aria-hidden="true">{icone}</span>
            )}
            <div>
